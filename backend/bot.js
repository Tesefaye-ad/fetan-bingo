 ...
 *
 * Deploy this as a second Render service (Background Worker, no public
 * port needed) with the same MONGO_URI as the API, plus the bot-specific
...
const BONUS_CONVERSION_RATE = Number(process.env.BONUS_CONVERSION_RATE || 1); // 1 bonus ETB -> this many ETB
...
// ---- in-memory step tracker for the tiny deposit/withdraw "wizards" ----
...
function mainKeyboard() {
  ...
  return Markup.keyboard([
    ...
    [Markup.button.text("Convert Bonus 💱")], // 👈 Convert Bonus ቁልፍ ተጨምሯል
  ])
    .resize() 
    .persistent(); 
}

// Finds the user, creating one on first contact so every command works even before /start.
...
// ---------------------------------------------------------------------
// /start - shows the banner + welcome text + persistent menu keyboard.
// ---------------------------------------------------------------------
bot.start(async (ctx) => {
  try {
...
// ---------------------------------------------------------------------
// Plain text handler for deposit/withdraw wizards
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// Render (and most PaaS hosts) expect every "Web Service" to bind to
// $PORT shortly after boot, or they assume the deploy failed and kill /
// restart the process - which is exactly what was aborting /start mid
// -sendPhoto ("socket hang up"). The bot itself doesn't need a port
// (it talks to Telegram via long polling), so this server only exists
// to satisfy Render's port check. If you deploy this as a Background
// Worker instead, this block is unnecessary but harmless.
...
async function main() {
  ...
  // 👈 ይህ ትዕዛዝ ሰማያዊውን የ "Menu" ቁልፍ ከቴክስት መጻፊያው አጠገብ እንዲመጣ ያደርጋል
  if (WEBAPP_URL) {
    try {
  ...
  console.log("[bot] Fetan Lottery bot is running");
}
